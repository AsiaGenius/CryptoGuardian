...
int parse_char(int fd)
{
	...
	while( RFIFOREST(fd) >= 2 )
	{
				...
				return 0;
		{
			...
			return 0;
			//CryptoGuardian by AsiaGenius
			ShowInfo("solicitacao de login - account_id:%d/login_id1:%d/login_id2:%d\n", account_id, login_id1, login_id2);
...
int Crypto_Check_Flag(int account_id)
{
	...
	if( SQL_ERROR == Sql_Query(sql_handle, "SELECT `flag` FROM `login` WHERE `account_id` = '%d'", account_id))
		Sql_ShowDebug(sql_handle);
